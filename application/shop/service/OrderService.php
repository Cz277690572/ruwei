...
class OrderService
{
    ...
    {
        return ['code' => 1, 'msg' => '门店订单创建成功！'];

        // 会员数据获取与检验
        if (!($member = Db::name('ShopMember')->where(['id' => $mid])->find())) {
        ...
        try {
            ...
            Db::transaction(function () use ($order, $orderList, $expressResult) {
                Db::name('ShopOrder')->insert($order); // 主订单信息
                Db::name('ShopOrderGoods')->insertAll($orderList); // 订单关联的商品信息
                Db::name('ShopOrderExpress')->insert($expressResult['data']); // 快递信息
            });
            // 同步商品库存列表
        ...
        return ['code' => 1, 'msg' => '门店订单创建成功！', 'order_no' => $order_no];
    }
    ...
    public static function buildExpressData(&$order, $address_id, $express_id)
    {
        ...
        if (!($address = Db::name('ShopMemberAddress')->field($addressField)->where($addressWhere)->find())) {
            return ['code' => 0, 'msg' => '收货地址数据异常！'];
        }
        // 物流信息查询
        $expressField = 'express_title,express_code';
        $expressWhere = ['id' => $express_id, 'status' => '1', 'is_deleted' => '0'];
        if (!($express = Db::name('ShopExpress')->field($expressField)->where($expressWhere)->find())) {
    ...
    private static function buildOrderData($item, &$order, &$orderList, $price_field = 'selling_price')
    {
        ...
        if (!($goods = Db::name('ShopGoods')->field($goodsField)->where($goodsWhere)->find())) {
        ...
        if (!($goodsSpec = Db::name('ShopGoodsList')->field($specField)->where($specWhere)->find())) {
